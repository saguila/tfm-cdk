...
yarn cdk deploy users-lake-formation-stack -c kaggleUser="$KAGGLE_UdSER" -c kaggleKey="$KAGGLE_KEY" -c awsAccount="$AWS_ACCOUNT_PRINCIPAL" -c dataSetName="$DATASET_NAME" -c landingDatabaseName="$LANDING_DATABASE" -c stagingDatabaseName="$STAGING_DATABASE" -c goldDatabaseName="$GOLD_DATABASE" -c dataLakeBucketName="$S3_BUCKET_NAME" --require-approval never
#yarn cdk destroy --all -c kaggleUser="$KAGGLE_USER" -c kaggleKey="$KAGGLE_KEY" -c awsAccount="$AWS_ACCOUNT_PRINCIPAL" -c dataSetName="$DATASET_NAME" -c landingDatabaseName="$LANDING_DATABASE" -c stagingDatabaseName="$STAGING_DATABASE" -c goldDatabaseName="$GOLD_DATABASE" -c dataLakeBucketName="$S3_BUCKET_NAME" --require-approval never
